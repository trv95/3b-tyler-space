Serves BugDB mock tickets (created-ticket style records). Supports exact filters plus substring match on `cve` and `subject`.

**Trigger:** HTTP `GET /vulndata/bugdb` â authenticated as `connector`, so other workflows in the tenant can call it through a workflow-backed connector pointing at this workflow.
